...
CONTRACT = ROOT / "contracts" / "ClaimGuardAI.py"


class ClaimGuardContractStaticTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_contract_has_nondet_consensus_and_payout(self):
        ...
        self.assertIn("def execute_payout", self.source)
    ...
    def test_storage_annotations_use_allowed_types(self):
        contract = next(
            node
            for node in self.tree.body
            if isinstance(node, ast.ClassDef) and node.name == "ClaimGuardAI"
        )
